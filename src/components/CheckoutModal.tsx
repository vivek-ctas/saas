...
import {
  ArrowLeft, ArrowRight, Check, CreditCard, Globe, Loader2,
  Lock, Shield, Smartphone, X, Zap,
} from 'lucide-react';
...
import type { Gateway, BillingCycle, CheckoutFormState, CheckoutStep, CreateLeadData } from '@/types/checkout.types';
...
const GATEWAYS: {
  ...
  methods: string; icon: React.ReactNode;
}[] = [
    {
      id: 'razorpay',
      label: 'Razorpay',
      badge: 'India',
      desc: 'UPI · PhonePe · Google Pay · Cards',
...
function displayPrice(plan: RazorpayPlan, cycle: BillingCycle, gateway: Gateway): string {
  if (plan.is_custom_pricing) return 'Custom';
  const isYearly = cycle === 'yearly';

  if (gateway === 'razorpay') {
    // Always INR
    const price = isYearly ? plan.yearly_price || plan.monthly_price * 10 : plan.monthly_price;
    return `₹${price.toLocaleString('en-IN')}`;
  } else {
    // USD — backend converts; show approximate
    if (plan.currency === 'USD') {
      const price = isYearly ? plan.yearly_price || plan.monthly_price * 10 : plan.monthly_price;
      return `$${price}`;
    }
    const rate = 83;
    const inrPrice = isYearly ? plan.yearly_price || plan.monthly_price * 10 : plan.monthly_price;
    const usd = Math.round((inrPrice / rate) * 100) / 100;
...
export default function CheckoutModal({
  plan, step, form, gateway, billingCycle, leadData, loading, error, isIndian,
  ...
  const priceDisplay = displayPrice(plan, billingCycle, gateway);
  const periodLabel = billingCycle === 'yearly' ? '/yr' : '/mo';
  const currencySymbol = gateway === 'razorpay' ? 'INR' : 'USD';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 leading-tight">
                {step === 'form' ? 'Get Started' : null}
                {step === 'summary' ? 'Payment Summary' : null}
                {step === 'processing' ? 'Processing…' : null}
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                {step === 'form' ? `${plan.name} plan selected` : null}
                {step === 'summary' ? `${plan.name} · ${billingCycle}` : null}
              </p>
            </div>
          </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      onFormChange('country_name', selected?.name || '');
                      // Auto-switch gateway based on country
                      onGateway(e.target.value === 'IN' ? 'razorpay' : 'stripe');
                    }}
                    className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                  >
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-slate-900">{leadData.currency}</span>
                  </div>
                  {plan.trial_days > 0 && (
                    <div className="flex justify-between text-sm">
                  ...
                  <div className="border-t border-slate-100 pt-3 flex justify-between items-baseline">
                    ...
                    <span className="text-2xl font-bold text-slate-900">
                      ...
                      {leadData.plan_price.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-semibold text-slate-500 tracking-widest uppercase">Payment Gateway</p>
                ...
                <div className="space-y-2.5">
                  {GATEWAYS.map((gw) => {
                    // Only show Razorpay option prominently for Indian users
                    // but always show both (Indian users may want Stripe for cards)
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {gw.badge && (
                              <Badge
                                variant="secondary"
                                className={`text-[10px] font-bold px-1.5 py-0 ${gw.id === 'razorpay' && isIndian
                                    ? 'bg-emerald-100 text-emerald-700'
                                    : 'bg-slate-100 text-slate-600'
                                  }`}
                              >
                                {gw.id === 'razorpay' && isIndian ? '✓ RECOMMENDED' : gw.badge}
                              </Badge>
                            )}
                          </div>
                ...
                {isIndian && (
                  <p className="mt-2.5 text-xs text-slate-500 text-center">
                    🇮🇳 Indian users can also use Stripe for international card payments
                  </p>
                )}
              </div>
            ...
            {step === 'summary' && (
              ...
              >
                {loading ? (
                  ...
                  <>
                    Pay {leadData?.currency === 'INR' ? '₹' : '$'}
                    {leadData?.plan_price?.toLocaleString()} with {
