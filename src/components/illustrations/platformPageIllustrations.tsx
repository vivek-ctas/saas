...
import { Boxes, ShoppingCart, FileText, BarChart3, Wand2, RefreshCw, LayoutGrid, ShieldCheck, Image as ImageIcon, Sparkles } from "lucide-react";
...
type IconKind = "box" | "bag" | "cart" | "tag" | "dot";

/** Reusable defs - drop once at the top of every illustration. */
const IllDefs = ({ id = "ill" }: { id?: string }) => (
    <defs>
...
const IllHeader = ({ x = 10, y = 10, label }: { x?: number; y?: number; label: string }) => (
    <g>
...
export const ChannelHeroDiagram = ({ cfg, ...props }: SVGProps<SVGSVGElement> & {
    ...
    const mapIcon = (icon: any): string => {
        ...
        if (icon === FileText) return "docCheck";
